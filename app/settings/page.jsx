...
const SettingBoxes = () => {
	...
	useEffect(() => {
		async function fetchData() {
			const response = await fetch(
				`http://localhost:3000/api/userprofile/${data?.user?.id}`
			);
	...
	const handleSubmit = async (e) => {
		setLoading(true);
		e.preventDefault();
		const result = await userProfileAction(name, contact, data?.user?.id);
							...
							<form onSubmit={handleSubmit}>
								<div className="mb-5.5">
									...
									<div className="relative">
										...
										<input
											className="w-full rounded-[7px] border-[1.5px] border-stroke bg-white py-2.5 pl-9 pr-4.5 text-dark focus:border-primary focus-visible:outline-none dark:border-dark-3 dark:bg-dark-2 dark:text-white dark:focus:border-primary"
											type="text"
											name="username"
											id="username"
											onChange={(e) => {
								...
								<div className="flex justify-end gap-3">
									<div>
										<button className=" text-white bg-[#4F46E5] hover:bg-[#433BCB] rounded-lg text-sm px-4 lg:px-5 py-3 lg:py-3.5 focus:outline-none font-extrabold w-full mt-3 shade mb-3 flex items-center justify-center">
